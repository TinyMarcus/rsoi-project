...
func InitConfig() {
	Config = Configuration{
		...
		KafkaConfig{Endpoints: []string{"localhost:9092"}, Topics: []string{"statistics"}},
		EndpointConfig{
			Statistics:       "http://localhost:8030",
			IdentityProvider: "http://localhost:8040",
			Loyalties:        "http://localhost:8050",
			Payments:         "http://localhost:8060",
			Reservations:     "http://localhost:8070",
		},
	}
}
