...
func (model *UserModel) RegisterUser(firstName, lastName, username, password, email string) (*objects.User, error) {
	user := &objects.User{
		FirstName: firstName,
		LastName:  lastName,
		Username:  username,
...
func (model *UserModel) GetUser(username string) (*objects.User, error) {
	user, err := model.repository.Find(username)
	if err != nil {
		return nil, err
	}
